...
| 5 | 동일 기준 비교 | ① 다섯 요소가 모두 같은 산업인가 ② **수치를 직접 비교할 때만** — 기간·단위·연결/별도·환율을 맞췄는가 |
| 6 | 최강 반론 생존 | 가장 센 반박을 먼저 대입하고도 야마가 남는가 |
| 7 | 공개 근거만으로 성립 | "추가 취재하면 알 수 있다"에 기대지 않는가 |

표기는 **`O` 확실 · `△` 애매 · `X` 미충족**, 등급은 `O=1 · △=0.5 · X=0` 합산으로 **A**(7.0점, 전 항목 `O`) · **B**(6.5점, `△` 1개) 둘뿐이다. 제시 기준을 만족하면 산술적으로 6.5점 이상일 수밖에 없어, 그 아래는 애초에 사용자에게 가지 않으므로 등급을 매기지 않는다. 이 판정표는 **후보 카드마다** 붙어 PDF에 실린다.
...
후보를 확정하기 전 **두 축으로** 점검한다.

- **보도량** — 그 사실을 한 달치로 검색해 같은 각도가 여러 매체에서 반복됐는지, 결과 전부터 다수가 같은 전망을 썼는지, 이미 기정사실인지
- **관심도** — 포털 랭킹 진입·조회수·좋아요·댓글 수·커뮤니티 확산·증권가 반복 언급 (확인 가능한 범위에서. 못 하면 보도량으로만 판단하고 그 사실을 밝힌다)

| 보도량 | 관심도 | 처리 |
|---|---|---|
| 적음 | 낮음 / 높음 | 그대로 진행 (관심은 많은데 안 다뤄진 소재는 오히려 좋다) |
| 많음 | 낮음 | 내린다 |
| **많음** | **높음** | **새 각도가 있을 때만 진행** |

관심이 많고 이미 많이 다뤄진 사안을 같은 각도로 한 번 더 쓰는 것은 발제가 아니다. 그 기사에서 **아직 아무도 조명하지 않은 다른 사실**을 찾아냈다면 소재가 겹쳐도 새 발제로 인정하고, 못 찾으면 버린다.
...
| 구간 | 범위 | 쓰임 |
|---|---|---|
| **핵심 창** | `T-7 ~ T0` | **트리거는 여기서 먼저 찾는다.** 발제 주제의 출발점은 최신일수록 좋다 |
| **확장 창** | `T-10 ~ T-7` | 핵심 창에서 5건을 못 채웠을 때만 여는 예비 구간. 여기서 온 트리거는 카드에 표시 |
| 창 밖 | `T-10` 이전 | 트리거 불가. **배경 통계·과거 비교·산업 맥락 자료로는 제한 없이 쓴다** |

원칙은 7일이다. 10일은 후보가 모자랄 때 여는 완충 구간이지 기본값이 아니다.
