...
/hb → [STEP 0 시각·도메인 확정 — 시스템 시각을 실제로 조회]
    ...
    → [STEP 1 최근 7일 이상징후 광역 탐색 — 언론·공시·IR·통계·정부·산업자료]
    → [STEP 2 귀납형 후보 생성 — 10체크 통과 상위 5건]
...
| 4 | 원인 확인 | 공식 원인이 확인되는가. 미공개라면 독립 공개자료 2건 이상과 반대 정황을 제시하고 인과를 단정하지 않는 `조건부 O`인가 |
| 5 | 동일 기준·시간축 비교 | 같은 산업인가, 직접 비교 기준이 같은가, 과거·경쟁사·업계 평균과 비교해 평소 대비 변화가 확인되는가 |
| 6 | 최강 반론 생존 | 가장 센 반박을 먼저 대입하고도 야마가 남는가 |
| 7 | 공개 근거만으로 성립 | "추가 취재하면 알 수 있다"에 기대지 않는가 |
| 8 | 이번 주의 변화 | 이번 주에 새로 달라진 것을 한 문장으로 답할 수 있는가 |
| 9 | 기존 기사와 차이 | 동일 결론의 반복이나 단순 수치 갱신이 아닌가 |
| 10 | 산업적 의미 | 계절성·기업 특수상황·산업 전체 자료와 대조해 의미가 남는가 |

각 항목을 0~10점으로 평가해 **O=8~10 · △=5~7 · X=0~4**로 표시하고, 기자 발제 중요도에 따라 100점으로 환산한다. 10개 모두 O여야 사용자에게 제시한다. 단, 4번은 직접 원인이 미공개라도 독립 기사·공시·통계 2건 이상이 같은 가능성을 가리키고, 반대 정황을 검색하며, 원인을 단정하지 않으면 **`조건부 O`(8점)** 로 통과한다. 통과 후보끼리는 점수 내림차순으로 1~5위를 매긴다.

**제시 기준은 엄격하다.** 내부 후보는 폭넓게 찾되 최종 출력은 10개가 모두 O인 상위 5건이다. 5건이 안 되면 7일 창 안에서 최소 두 차례 재탐색하고도 부족할 때 통과분만 제시한다. 가능하면 3건 이상을 목표로 하지만 약한 후보로 채우지 않는다.
...
### 체크 5가 `O`가 아니면 잘라내고 재탐색한다

보류하거나 조건부로 남기지 않는다. 후보를 목록에서 지우고 **빈자리를 채우러 다시 검색한다.**

```
5번 △ 또는 X → 후보 삭제 → 원인 특정(산업 불일치 / 비교 기준 불일치)
  → 질문을 트리거 산업 안으로 좁혀 근거 재수집
  → 안 되면 트리거를 버리고 새 소재로 재검색 → 재판정
```

**재탐색은 최소 한 번 반드시 돈다.** "애매해서 뺐습니다"로 끝내고 후보 수를 줄이는 것은 허용하지 않는다. 잘라낸 이력은 남긴다 — "산업 불일치 2건 잘라내고 재탐색해 1건 보충".

### 트리거는 최근 7일로 고정한다

| 구간 | 범위 | 쓰임 |
|---|---|---|
| **트리거 창** | `T-7 ~ T0` | 발제 출발점이 되는 사건·발표·수치 변화는 이 구간에서만 찾는다 |
| 창 밖 | `T-7` 이전 | 트리거 불가. **독립 근거·과거 비교·산업 맥락·반론 자료로는 제한 없이 쓴다** |

후보가 모자라도 트리거 창을 10일로 늘리지 않는다. 7일 안에서 검색어·업종·자료 유형을 바꿔 재탐색한다.

### 체크 5의 ②항은 비교할 때만 적용한다

독립 근거는 서로 다른 출처에서 와야 하는데(체크 2), 출처가 다르면 기간·단위가 다른 게 정상이다. 그래서 **②항은 수치를 직접 견줄 때만** 본다.

> **비교에 해당** — "A사 1조, B사 8000억으로 A사가 앞선다" / "전년 동기 대비 20% 늘었다"
> **비교 아님** — "관세청 집계로 수출이 65.3% 늘었다. 4사의 상반기 영업이익은 14조원이다." → 견주지 않고 각각 제시

비교하지 않으면 ②는 해당 없음이고 ①(산업 일치)만으로 `O`다. 다만 기간이 다른 수치를 나란히 실을 때는 **각각 무엇을 언제 측정한 것인지** 밝힌다.
...
2. **발제 후보 전체** — 선택되지 않은 것까지 전부, 선정된 건에 `★` 표시
   각 카드에 발제 주제 · 조명한 사실 · 트리거 원문 출처와 **발행 날짜·시각** · 독립 근거 요약과 각 출처 · 반대·제한 근거 · 말할 수 있는 범위 · **산업 정합성과 10체크 판정표(O·△·X), 100점 환산 점수와 순위**
3. **선정 발제 검증** — 확정 야마, 10체크 표, 최강 반론과 반박
4. **완성 기사** — 제목 · 부제 · 본문 전문
5. **출처 일람** — 번호순 전체 목록
...
- **유료 기사·비공개 리포트는 못 읽는다.** 10체크의 7번이 "공개 근거만으로 성립"인 이유다.
