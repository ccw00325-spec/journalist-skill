...
표기는 **`O` 확실 · `△` 애매 · `X` 미충족**, 등급은 `O=1 · △=0.5 · X=0` 합산으로 **A**(6.0점↑) · **B**(4.5점↑) · **C**(미만, 제시하지 않음). 이 판정표는 **후보 카드마다** 붙어 PDF에 실린다.

**제시 기준은 엄격하다.** 후보는 **최소 5건이되 전부 고품질**이어야 한다 — 체크 1·2·5는 `O` 필수(5번은 `△`도 불가), 나머지 4개 축에서 `△`는 최대 1개. 기준을 못 넘은 후보는 어차피 쓸 수 없으므로 **카드로 싣지 않고 제외 건수·사유만 한 줄로** 밝힌다.

판정표에는 **트리거 날짜 열**이 있고 표 아래에 각 근거의 날짜가 붙는다. 얼마나 최신 자료로 뽑은 발제인지 한눈에 보인다.

6번은 형식적으로 처리하지 않는다. 반대편 근거를 **실제로 검색해서** 찾고, 무너지면 GATE 2로 되돌아갈지 사용자에게 묻는다.

### 발제와 발생은 다르다

**발생**은 사건이 일어난 것이고, **발제**는 그 사건 속 여러 사실 중 **흥미로울 만한 단 하나를 골라 조명하고, 그 하나를 떠받칠 근거를 밖에서 모아 오는 것**이다.
...
FACT를 몇 개 엮지도 않은 것은 발제가 아니다. **조명한 사실 1개 + 트리거 밖 독립 근거 2건 이상**이 최소 요건이다.

### 묶는 방향을 두 가지로 낸다

같은 근거라도 어떻게 묶느냐에 따라 기사가 달라진다. 한 방향만 내밀면 사용자가 그 틀에 갇히므로, 후보마다 두 가지를 함께 낸다.

- **방향 A — 사실 그대로** : 확인된 사실을 덧붙임 없이 나열하는 각도
- **방향 B — 다르게 묶기** : 같은 근거를 다른 축으로 재배열한 각도

둘 다 확인된 FACT 범위 안에서만 만든다. B가 성립하지 않으면 그 사실을 밝히고 A만 낸다.

### 이미 기사화가 많이 된 사안은 피한다

한 달쯤 전부터 다수 언론이 결과를 미리 점쳐 왔거나 이미 기정사실이 된 사안은 새로 조명할 여지가 없다. 후보를 확정하기 전 그 사실을 **한 달치로 검색해** 과포화 여부를 점검하고, 걸리면 내린 뒤 사유를 밝힌다. 다만 **아직 아무도 조명하지 않은 다른 사실**을 그 기사에서 찾아냈다면 소재가 겹쳐도 새 발제로 인정한다.

### 산업 정합성 — 다섯 요소가 같은 산업이어야 한다

**발제 · 트리거 · 조명한 사실 · 핵심 질문 · 독립 근거**가 전부 같은 산업 안에 있어야 한다. 하나라도 새면 기사로 못 쓴다.

트리거가 조선사의 발전용 엔진 수주인데 질문을 "미국 전력 인프라 수요"로 넓혀 근거를 전력기기·전선에서 가져오면, 세 산업이 섞여 기사가 성립하지 않는다. 이때는 질문을 트리거의 산업 안으로 좁혀 근거를 다시 찾거나 후보를 버린다.

밸류체인 연결(조선-조선기자재 등)은 **납품 관계가 자료로 확인될 때만** 같은 산업으로 인정한다. "둘 다 중후장대", "둘 다 미국 수요" 같은 느슨한 공통점으로 묶지 않는다.

### 과잉해석 방지 — 근거를 종합할 때

개별 FACT가 사실이어도 그것들을 이어 만든 결론까지 자동으로 사실이 되지는 않는다. 그래서 **개별 FACT 확인**과 **종합 해석 검증**을 분리하고, 산업 전체 자료(시장 규모·성장률·기업 비율·기관 통계)로 대조한다.

- **대표성 8문항** — 같은 기업 반복, 같은 보도자료 받아쓰기, 일부 기업만 선택, 세부 분야를 전체로, 국내를 글로벌로, 단기를 구조 변화로, 계획을 시장 형성으로 부풀리지 않았는지 점검
- **강도 제한** — `산업 전반 · 본격화 · 급증 · 확산 · 시장 재편 · 앞다퉈 · 잇따라 · 속속` 류는 산업 전체를 보여주는 자료가 있을 때만
- **반대 사례 탐색** — 철수·투자 축소·매출 미발생·사업 지연·시장 정체를 의도적으로 검색해 판단에 포함
- **Reality Check** — 제출 직전 8단계를 순서대로 통과시키고 "업계 종사자가 읽어도 과장이 아닌가"에 답한다

> **발제가 약해지는 편이 잘못된 발제를 만드는 것보다 낫다.**
...
<작업폴더>/HB_Output/2026_08_12_2246output.html    ← 수정·재출력용 원본
```

파일명은 `YYYY_MM_DD_HHmmoutput` — 연도·월·일·시분. 시각까지 들어가서 같은 날 여러 번 돌려도 겹치지 않고, 같은 분에 부딪히면 `_2` 가 붙어 앞 결과를 덮어쓰지 않는다.
...
2. **발제 후보 전체** — 선택되지 않은 것까지 전부, 선정된 건에 `★` 표시
   각 카드에 발제 주제 · 조명한 사실 · 트리거 원문 출처와 **발행 날짜·시각** · 독립 근거 요약과 각 출처 · 반대·제한 근거 · 말할 수 있는 범위 · **산업 정합성과 7체크 판정표(O·△·X)와 등급**
