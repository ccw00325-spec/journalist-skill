...
**판단 기준은 관심도 하나다.** 기사가 몇 건 나왔는지는 보지 않는다. 여러 매체가 받아썼어도 아무도 안 읽었으면 독자에게는 새 이야기이고, 한 건만 나왔어도 모두가 읽었으면 이미 아는 이야기다.

포털 랭킹 진입·조회수·좋아요·댓글 수·커뮤니티 확산·증권가 반복 언급을 확인 가능한 범위에서 본다.

| 관심도 | 처리 |
|---|---|
| 낮음 | 그대로 진행 |
| 확인 못 함 | 그대로 진행 — 카드에 "관심도 확인 못 함" 표시 |
| **높음** | **새 각도가 있을 때만 진행** |

관심이 높은 사안을 같은 각도로 한 번 더 쓰는 것은 발제가 아니다. 그 기사에서 **아직 아무도 조명하지 않은 다른 사실**을 찾아냈다면 소재가 겹쳐도 새 발제로 인정하고, 못 찾으면 버린다. 지표를 확인할 수 없으면 **추정하지 않는다** — 못 읽었으면 못 읽었다고 쓰고 진행한다.
